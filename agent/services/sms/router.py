...
class SMSRouter:
    ...
    def __init__(
        ...
        *,
        ...
        on_command_stop: SMSEventHandler | None = None,
        on_command_help: SMSEventHandler | None = None,
        on_command_unsub: SMSEventHandler | None = None,
        ...
        self._on_command_stop = on_command_stop or self._noop
        self._on_command_help = on_command_help or self._noop
        self._on_command_unsub = on_command_unsub or self._noop
    ...
    async def route(self, event: InboundSMSEvent) -> None:
        if event.event_type == "inbound_sms":
            ...
            return
        if event.event_type == "command_stop":
            await self._on_command_stop(event)
            return
        if event.event_type == "command_help":
            await self._on_command_help(event)
            return
        if event.event_type == "command_unsub":
            await self._on_command_unsub(event)
            return
